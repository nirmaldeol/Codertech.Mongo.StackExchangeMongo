...
namespace Abp.Caching.MongoCache
{
    public class MongoContext
    {
        ...
        private static FilterDefinition<CacheItem> FilterByKeys(string[] keys)
    => Builders<CacheItem>.Filter.In(x => x.Key, keys);
        ...
        public void RemoveMany(string[] keys)
          => _collection.DeleteMany(FilterByKeys(keys));
        public async Task RemoveManyAsync(string[] keys, CancellationToken token = default)
          => await _collection.DeleteManyAsync(FilterByKeys(keys), token);
